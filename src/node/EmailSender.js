...
exports.sendTemplate = function(templateName, data, sendTo){
    console.log(`preparing email template: ${templateName} for ${sendTo}`)
...
async function findRecipient(to){
    ...
    if (!to.email){
        Raven.captureException(new Error("Error finding recipient: no \"email\" address was set"))
    }
...
function getActualRecipients(original, config){
    if (config.get("emailOverrideEnabled")) {
        ...
        for (var i = 0; i < overrideEmails.length; i++) {
            overrides.push({email: overrideEmails[i], name: original.name || original.email});
        }
        return overrides;
    }
    else {
